...
这是一个严格使用 Modern libxposed API 102 的 LSPosed 模块，为超级小爱 `8.0` 及以上版本提供
第三方 Streamable HTTP/SSE MCP、Agent 完整轨迹、System Prompt 补丁、模型首次输出超时和目录级文件权限等增强能力。
它不会修改或重签名超级小爱 APK。
...
再自动重新打开；正在进行的对话、模型响应和工具调用会立即中断。首次使用时需在 KernelSU、
Magisk 等 root 管理器中授权，拒绝授权或命令执行失败时模块不会用普通启动伪装成重启。
...
模块 App 的“模型首次输出超时”可以控制超级小爱等待模型首次可见输出的 watchdog：

- “跟随宿主”不覆盖目标应用的设置；参考版默认是 120 秒；
- “自定义”接受大于 0 的整数秒；
- “不限制”取消这一层首次输出计时。

这里的“首次可见输出”指正文增量、完整工具调用或模型结束事件；公开 reasoning 本身不算作
首次可见输出。设置保存后需要结束并重新启动超级小爱进程。该设置不会修改宿主的流式输出
空闲超时、单次模型请求绝对上限或 MCP 服务器自身的网络/工具超时，因此“不限制”不表示
关闭超级小爱的所有超时保护。

未配置、配置损坏、目标解析失败或候选不唯一时，模块保持宿主原有限制。超时目标与其他能力
一样按已验证 profile、DexKit 定向发现、完整 DEX 结构扫描的顺序独立解析。
